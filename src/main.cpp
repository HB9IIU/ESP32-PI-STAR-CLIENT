...
char g_lastActivityBannerText[3] = "";
...
void buildFooterStatusText(char *destination, size_t destinationSize)
{
    ...
    char slotText[8] = "TS-";
    char heardText[20] = "0 heard";

    buildFooterFrequencyText(frequencyText, sizeof(frequencyText));

    if (g_live.slot_valid)
    {
        snprintf(slotText, sizeof(slotText), "TS%d", g_live.slot);
    }
    else if (g_lastFooterSlotValid)
    {
        snprintf(slotText, sizeof(slotText), "TS%d", g_lastFooterSlot);
    }

    if (g_heardSummary.valid)
    {
        snprintf(heardText, sizeof(heardText), "%d heard", g_heardSummary.unique_callsigns_total);
    }

    snprintf(destination, destinationSize, "%s | %s | %s", frequencyText, slotText, heardText);
}
...
void updateFooterStatusTextDisplay(const char *statusText, uint16_t textColor)
{
    const char *nextText = (statusText != nullptr && statusText[0] != '\0') ? statusText : "--.--- MHz | TS- | 0 heard";
    ...
    if (g_lastFooterStatusText[0] != '\0')
    {
        drawFooterStatusText(g_lastFooterStatusText, TFT_BLACK);
    }
...
void setup()
{
    ...
    {
        buildWiFiConnectingStatusText(wifiStatusText, sizeof(wifiStatusText), wifiConnectStartMs);
        updateFooterStatusTextDisplay(wifiStatusText, wifiStatusColor);
        delay(500);
        Serial.print(".");
    }
    snprintf(wifiStatusText,
             sizeof(wifiStatusText),
             "WiFi connected %s",
